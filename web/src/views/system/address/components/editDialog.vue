<template>
    <el-dialog title="修改地址" :visible.sync="editDialogVisible" center custom-class="addressWidth">
...
<script>
    import {updateAddress, envList} from "@/api/api";
    ...
    export default {
        ...
        watch: {
            value(val) {
                ...
                let env
                for (let i = 0; i < this.envOption.length; i++) {
                    //筛选出匹配数据，是对应数据的整个对象
                    if (this.envOption[i].label === this.$parent.rowData.env){
                       env = this.envOption[i].value
                    }
                }
                this.formData = {
                    env: env,
                    name: this.$parent.rowData.name,
                    host: this.$parent.rowData.host
                }
            },
