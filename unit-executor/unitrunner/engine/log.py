...
class CaseRunLog:
    ...
    def save_extractors(self, name, ext_type, expression, value, result, state) -> None:
        if not hasattr(self, 'data_extractor'):
            setattr(self, 'data_extractor', [])
        info = {"vars_name": name, "type": ext_type, "expression": expression, "result_val": value, 'result': result, 'state': state}
