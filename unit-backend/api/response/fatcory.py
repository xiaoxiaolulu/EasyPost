...
class ResponseStandard:
    ...
    def resp_400(code: int = 400, data: str = None, msg: str = "请求错误(400)❌") -> dict:
        return dict(status_code=status.HTTP_400_BAD_REQUEST, content={'code': code, 'message': msg, 'data': data})

    @staticmethod
    def resp_401(*, data: str = None, msg: str = "未授权，请重新登录(401)❌") -> dict:
        return dict(status_code=status.HTTP_401_UNAUTHORIZED, content={'code': 401, 'message': msg, 'data': data})

    @staticmethod
    def resp_403(*, data: str = None, msg: str = "拒绝访问(403)❌") -> dict:
        return dict(status_code=status.HTTP_403_FORBIDDEN, content={'code': 403, 'message': msg, 'data': data})

    @staticmethod
    def resp_404(*, data: str = None, msg: str = "请求出错(404)❌") -> dict:
        return dict(status_code=status.HTTP_404_NOT_FOUND, content={'code': 404, 'message': msg, 'data': data})

    @staticmethod
    def resp_422(*, data: str = None, msg: typing.Union[list, dict, str] = "不可处理的实体❌") -> dict:
        return dict(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, content={'code': 422, 'message': msg, 'data': data})
